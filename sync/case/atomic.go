...
func AtomicCase() {
	...
	//count += 10
	fmt.Println("原有基础上累加后变量的值 ", atomic.AddInt64(&count, 10))

	fmt.Println("交换变量的值，并返回原有的值:  ", atomic.SwapInt64(&count, 30))
	fmt.Println("交换之后，获取变量的值 ", atomic.LoadInt64(&count))

	// 比较并替换原有的值，并返回是否替换成功。通过旧值对比，如果相等，则进行替换
	fmt.Println("比较并替换原有的值，并返回是否替换成功:  ", atomic.CompareAndSwapInt64(&count, 30, 40))
	fmt.Println("比较替换之后，获取变量的值 ", atomic.LoadInt64(&count))
...
func AtomicCase2() {
	...
	for i := 0; i < 100; i++ {
		...
		atomicLabel:
			...
			success := atomicMp.CompareAndSwap(m, &m1)
			if !success {
				goto atomicLabel
			}
		}()
